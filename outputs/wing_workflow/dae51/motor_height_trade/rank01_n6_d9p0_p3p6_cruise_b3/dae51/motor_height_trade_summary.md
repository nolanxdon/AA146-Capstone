...
- Lowest equivalent stall speed in the sweep occurred at `42 mm` drop: `4.923 m/s`.
- `CLmax` effectively saturated by about `0 mm` drop (`0.00 c`).
- The strongest modeled post-stall harshness occurred at `70 mm` drop with `poststall_drop_5deg = 3.681`.

### Slotted flap + blowing

- Lowest equivalent stall speed in the sweep occurred at `42 mm` drop: `4.637 m/s`.
- `CLmax` effectively saturated by about `0 mm` drop (`0.00 c`).
- The strongest modeled post-stall harshness occurred at `84 mm` drop with `poststall_drop_5deg = 4.035`.
